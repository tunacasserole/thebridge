// GitHub Merged PRs API Route - Server-side only
import { NextResponse } from 'next/server';
import { fetchMergedPRsMultiRepo } from '@/lib/github';
...
export async function GET() {
  ...
  try {
    const data = await fetchMergedPRsMultiRepo(token, owner, repos);
    return NextResponse.json({ success: true, data });
  } catch (error) {
    console.error('[GitHub Merged PRs API] Error:', error);
    return NextResponse.json(
      {
        success: false,
